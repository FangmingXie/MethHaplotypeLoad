...
if [ "$outputFormat" == "test" ]; then
	...
	samtools view -F 4 -f 2 $bam "$chr:$start-$end" \
		...
		'BEGIN{FS=OFS="\t" 
			...
			{
				# readname $1, chr $3, start $4, sequence $14
				gsub("XM:Z:", "", $14) 
				len=length($14)
				# max
				lstart=0
				...
				if (lastReadName==$1) {
					...
					currEnd=$4-1+lend
					currSeq=substr($14, lstart+1, lend-lstart)
					...
					lastEnd=$4-1+lend
					lastSeq=substr($14, lstart+1, lend-lstart)
				}
	...
	samtools view -F 4 -f 2 $bam "$chr:$start-$end" \
		...
		'BEGIN{FS=OFS="\t" 
			  ...
			  } 
			{
				...
				# test if read name is the same
				if (lastReadName==$1) {
					print "!"$3, $4-1+lstart, $4-1+lend, $1, substr($14, lstart+1, lend-lstart)
				}
				else {
					print $3, $4-1+lstart, $4-1+lend, $1, substr($14, lstart+1, lend-lstart)
				}
